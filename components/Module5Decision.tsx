...
const Module5Decision = ({ onComplete }: Module5DecisionProps) => {
  ...
  const handleCardClick = (id: string) => {
    if (selectedCard !== null) return; // lock further taps once one is picked
    ...
    // Correct pick → flip just that card.
    // Wrong pick → flip it AND the success card, revealing the right answer.
    const idsToFlip =
      card.type === "success"
        ? [card.id]
        : cardsData
            .filter((c) => c.id === id || c.type === "success")
            .map((c) => c.id);

    setFlippedCards(
      idsToFlip.reduce<{ [key: string]: boolean }>((acc, cardId) => {
        acc[cardId] = true;
        return acc;
      }, {}),
    );
