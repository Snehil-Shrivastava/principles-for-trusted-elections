...
const Module4Screen5 = ({ onComplete }: Module4Screen5Props) => {
  ...
  const handleSelectCard = (id: string) => {
    ...
    // Wrong pick → flip everyone (full context on why).
    // Right pick → flip only the success-type cards (the other right answer),
    // leaving the wrong card untouched.
    const idsToFlip =
      card.type === "error"
        ? cardsData.map((c) => c.id)
        : cardsData.filter((c) => c.type === "success").map((c) => c.id);

    setFlippedIds(idsToFlip);
  };

  // Flip + resize every card in flippedIds together, on one timeline
  useGSAP(() => {
    ...
    flippedIds.forEach((id) => {
      ...
      const flipperEl = flipperRefs.current[id];
      if (!cardEl || !flipperEl) return;
